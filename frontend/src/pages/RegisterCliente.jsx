  ...
  return (
    <div className="min-h-screen bg-slate-50 py-10">
      <div className="mx-auto max-w-2xl rounded-2xl border border-slate-200 bg-white p-8 shadow-lg">
        <h1 className="text-2xl font-bold text-slate-900">Cadastro de cliente</h1>
        <p className="mt-1 text-sm text-slate-500">Preencha seus dados e até três rendimentos.</p>
        <form onSubmit={onSubmit} className="mt-8 space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="text-sm font-medium text-slate-700">Login (CPF) *</label>
              <input
                required
                className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2"
                value={form.login}
              ...
              <div key={i} className="mt-4 rounded-lg border border-slate-200 p-4">
                <div className="flex justify-end">
                  {rendimentos.length > 1 && (
                    <button type="button" className="text-xs text-red-600" onClick={() => removeRend(i)}>
                      Remover
                    </button>
                  )}
                </div>
