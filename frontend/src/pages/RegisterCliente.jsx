  ...
  async function onSubmit(e) {
    ...
    try {
      await register({
        ...
        idAgente: form.tipoUsuario === 'AGENTE' && form.idAgente !== '' ? Number(form.idAgente) : null,
      });
      navigate('/login');
    } catch (ex) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-10">
      <div className="mx-auto max-w-2xl rounded-2xl border border-slate-200 bg-white p-8 shadow-lg">
        ...
        <p className="mt-1 text-sm text-slate-500">Escolha se deseja cadastrar como cliente ou agente.</p>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {form.tipoUsuario === 'CLIENTE' ? (
              ...
              <>
                ...
                <div>
                  <label className="text-sm font-medium text-slate-700">ID do agente (opcional)</label>
                  <input
                    type="number"
                    className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2"
                    value={form.idAgente}
